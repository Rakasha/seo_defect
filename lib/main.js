...
/**
 * Detector which accept HTML and validation rules and generates the reports
 * @class
 */
class Detector {
  /**
  ...
  /**
   * Set the HTML source
   * @param  {String} src - URL or filepath of the HTML
   * @return {Promise}
   */
  setDocumentSource(src) {
    if (isURL(src)) {
      return this.setDocumentSourceFromURL(src);
    } else {
      return this.setDocumentSourceFromFile(src);
    }
  }
